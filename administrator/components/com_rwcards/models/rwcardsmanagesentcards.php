...
class RWCardsModelRWCardsManageSentCards extends JModel
{
	...
	*/
	...
	/**
	* Method to delete all records that are older then defined in the configuration table
	*
	* @access private
	*/
	function delete()
	{
		...
		// retrieve component parameters from the back end this way:
		$params =& JComponentHelper::getParams( 'com_rwcards' );

		$lifetime = $params->get( "lifetime", 7 );
		$lifetimeDate = date("Y-m-d", time() - $lifetime * 24*60*60 );
		
		$db->setQuery( "DELETE FROM #__rwcardsdata WHERE writtenOn <= '" . $lifetimeDate . "'");
		$db->query();
	}
