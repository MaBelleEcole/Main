...
class RWCardsModelRWCardsManageEcards extends JModel
{
	...
	*/
	...
	{	
		...
		$this->_data['filter_order_Dir'] = $filter_order_Dir;
		...
		if ( $search ) 
		{
			$where1[] = 'autor LIKE "%'.$db->getEscaped($search).'%" or description LIKE "%'.$db->getEscaped($search).'%"';
			$where[] = 'autor LIKE "%'.$db->getEscaped($search).'%" or description LIKE "%'.$db->getEscaped($search).'%"';
		}
		if ( $filter_catid ) 
		{
			$where1[] = 'id = '.(int) $filter_catid;
		...
		if ( $filter_state ) 
		{
			if ( $filter_state == 'P' ) 
			{
				$where1[] = '#__rwcards.published = 1';
				$where[] = '#__rwcards.published = 1';
			}
			else if ($filter_state == 'U' ) 
			{
				$where1[] = '#__rwcards.published = 0';
		...
		{
			$where1[] = "#__rwcards.category_id = '" . $category_id . "'";
		...
		$query = "SELECT COUNT(*) FROM #__rwcards" . $where1 . $orderby;
		$db->setQuery( $query );
	...
	*/
	function &getEditData()
	{
		...
		{
			if ( preg_match("/.*" . $suffix . ".*/", $file) ) continue;
	...
	*/
	...
	function RWCardsCreateThumbnails()
	{
		...
		$name = strtolower( substr($image[0], 0, -4) ) . $suffix . $fileExtension; 
