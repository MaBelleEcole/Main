	...
	<table>
		<tr>
			<td align="left" width="100%">
				...
				<input type="text" name="search" id="search" value="<?php echo htmlentities( $this->rwcardsCategories['lists']['search'], ENT_QUOTES, 'UTF-8' );?>" class="text_area" onchange="document.adminForm.submit();" />
		...
		?>
					...
					<td>
						<?php
						if ( $row[$i]->checked_out && ( $row[$i]->checked_out != $user->id ) ) {
							echo htmlentities( $row[$i]->category_kategorien_name, ENT_QUOTES, 'UTF-8' );
						} else {
							?>
							<a href="<?php echo $link; ?>" title="<?php echo JText::_( 'EDIT ENTRY' ); ?>">
								<?php echo htmlentities( $row[$i]->category_kategorien_name, ENT_QUOTES, 'UTF-8' ); ?></a>
							<?php
						}
						?>
					</td>
					<td>
						<?php echo nl2br( htmlentities( $row[$i]->category_description, ENT_QUOTES, 'UTF-8' )); ?>
					</td>
					<td align="center">
						<?php echo $published;?>
					</td>
					<td class="order">
						<span><?php echo $this->rwcardsCategories['_pageNav']->orderUpIcon( $i, ( ($i != 0) /* && $row[$i]->id == @$row[$i-1]->id */), 'orderup', 'Move Up', $ordering ); ?></span>
						<span><?php echo $this->rwcardsCategories['_pageNav']->orderDownIcon( $i, $n, ( ($i != $n - 1) /* && $row[$i]->id == @$row[$i+1]->id */ ), 'orderdown', 'Move Down', $ordering ); ?></span>
